...
export default function Heun() {
  const { settings } = useSettings()
  const { push: pushHistory } = useHistory()
  const [searchParams] = useSearchParams()

  const [f, setF] = useLocalStorage('Heun_f', '')
  const [x0, setX0] = useLocalStorage('Heun_x0', '')
  const [y0, setY0] = useLocalStorage('Heun_y0', '')
  const [h, setH] = useLocalStorage('Heun_h', '')
  const [n, setN] = useLocalStorage('Heun_n', '')
  ...
  async function calcular() {
    ...
    try {
      ...
      const data = await apiPost('edos/heun', payload)
  ...
  const teoria = (
    <Expander title="¿Cómo funciona el método de Heun?">
      <p>
        <strong>Concepto básico:</strong> Es un método predictor-corrector (Runge-Kutta de 2do orden) que mejora a Euler. 
        Predice el siguiente punto usando Euler y luego promedia las pendientes al inicio y al final del intervalo.
      </p>
      <br />
      <p><strong>Fórmulas:</strong></p>
      <Latex tex={String.raw`k_1 = f(x_n, y_n)`} display />
      <Latex tex={String.raw`k_2 = f(x_n + h, y_n + h \cdot k_1)`} display />
      <Latex tex={String.raw`y_{n+1} = y_n + \frac{h}{2} (k_1 + k_2)`} display />
      <br />
    </Expander>
  )
  ...
  const code = `def heun(f, x0, y0, h, n):
    x, y = x0, y0
    puntos = [(x, y)]
